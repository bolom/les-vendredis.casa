---
layout: article
title: "The deck is done."
summary: "We'd been saying it since last year. A friend had a fall on it without getting hurt, and the deck finally became urgent."
description: "The new wooden deck on the Les Vendredis A-frame in Martinique: a fast build, fresh timber, first guests, the view onto Diamond Rock, and Easter weekend."
tag: "Build log"
date: 2026-05-29
image: "/images/articles/planches-terrasse-cabane-aframe-chantier-les-vendredis-04.webp"
image_alt: "New planks laid out during the build"
lang: "en"
translation_key: "le-deck-est-fait"
published: true
permalink: /journal/the-deck-is-done/
---

We'd been saying we'd build it since last year.

What was missing: the moment, the energy, the wood, the decision.

Last weekend, a friend had a fall on it — no injury. There was the decision.

![Timber stacked in front of the A-frame before the build]({{ '/images/articles/terrasse-bois-cabane-aframe-chantier-les-vendredis-01.webp' | relative_url }})

Friday morning, we went to get the wood. Three hours, there and back.

By the afternoon, we were on it.

![Ladder and timber in front of the cabin during the build]({{ '/images/articles/echelle-terrasse-bois-cabane-aframe-les-vendredis-02.webp' | relative_url }})

It wasn't just a few planks to lay down. It was the piece missing between the cabin and the garden — where you put your feet stepping out, where you sit, where the view opens up.

![A-frame structure and the opening toward the future deck]({{ '/images/articles/charpente-aframe-echelle-travaux-terrasse-les-vendredis-03.webp' | relative_url }})

Measure. Cut. Align. Start over.

Carry the planks one by one, check the levels, redo it anyway — because we'll see it every day.

![New planks laid out during the build]({{ '/images/articles/planches-terrasse-cabane-aframe-chantier-les-vendredis-04.webp' | relative_url }})

By that evening, it was done.

Not perfect. Built by hand, on time, solid. With the smell of fresh wood that changes everything.

![Laying the deck with greenery and the sea behind]({{ '/images/articles/pose-terrasse-bois-vue-mer-les-vendredis-05.webp' | relative_url }})

The next day at 4pm, our first guests arrived — one night, from Fort-de-France. Deck still pale, still some construction dust in the corners.

![Finished deck, tools still out in front of the cabin]({{ '/images/articles/terrasse-bois-outils-cabane-aframe-les-vendredis-06.webp' | relative_url }})

They left on Monday and wrote this:

> *"An unusual place, close to nature, built with a lot of love and you can feel it. Beautiful garden that makes you want to enjoy the spot. Flawless welcome and lovely encounters. We recommend it."*

The week kept the same rhythm.

Thursday: planting, harvesting maracudja and breadfruit. A quiet lunch, just us.

![Passion fruit freshly harvested on the ground]({{ '/images/articles/maracudja-recolte-01.webp' | relative_url }})

We tried cooking the breadfruit straight in the fire. Epic fail. We'll try again.

![Marble checkerboard on the deck — a gift from Yolande]({{ '/images/articles/jeu-dames-yolande.webp' | relative_url }})

It's no longer a cabin sitting in the garden. It's a threshold. A place to arrive and stay.

![View from inside the cabin toward the deck, the palms, and Diamond Rock]({{ '/images/articles/interieur-cabane-aframe-terrasse-vue-rocher-diamant-les-vendredis.webp' | relative_url }})

The deck is there. So is the view.
